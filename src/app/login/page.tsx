...
import { Mail, ArrowRight, Loader2, ShieldCheck, CheckCircle2 } from 'lucide-react';
...
function LoginContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/auth/magic-link?email=${encodeURIComponent(email)}`, {
        method: 'POST'
      });
    ...
    <div className="w-full max-w-md space-y-8 z-10">
        <div className="text-center space-y-6">
          ...
          <div className="space-y-2">
            <h1 className="text-4xl font-black tracking-tighter uppercase italic text-white">Acesso Restrito</h1>
        ...
        {!sent ? (
            ...
            <div className="space-y-4">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-12 pr-4 text-white focus:border-white focus:bg-white/10 outline-none transition-all placeholder:text-zinc-700"
                />
              </div>
            </div>
            <button
              disabled={loading || !email}
              className="w-full bg-white text-black py-4 rounded-2xl font-black text-lg flex items-center justify-center gap-3 hover:scale-[1.02] active:scale-95 transition-all shadow-[0_10px_40px_rgba(255,255,255,0.15)] disabled:opacity-50"
            >
          ...
          <div className="text-center space-y-8 p-10 border border-white/10 bg-zinc-950/80 rounded-[2.5rem] backdrop-blur-xl relative overflow-hidden">
            ...
            <div className="space-y-4">
              <h2 className="text-3xl font-black uppercase tracking-tighter text-white">Chave Enviada!</h2>
              <p className="text-zinc-400 leading-relaxed text-sm text-white">
                Enviamos uma chave de acesso segura para <br/><strong className="text-white">{email}</strong>
              </p>
            </div>
            <div className="space-y-4 pt-6 border-t border-white/10">
              <p className="text-xs text-zinc-400">Verifique sua caixa de entrada e clique no botão para entrar.</p>
              <button 
                onClick={() => setSent(false)}
                className="text-xs text-zinc-500 uppercase font-black tracking-widest hover:text-white transition-colors"
              >
